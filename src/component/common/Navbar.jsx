...
function Navbar() {
    ...
    // Close dropdown/menu/search on Escape key
    useEffect(() => {
    ...
    const handleMobileMenuToggle = () => {
        ...
        setShowDropdown(false); // Close dropdown if mobile menu opens
    };
                ...
                <div className="flex justify-between items-center">
                    ...
                    <div className='hidden lg:flex gap-4 items-center'>
                        <ul className='flex gap-2 items-center'>
                            {NavLink.map((item, id) => (
                                <li key={id}>
                                    <Link to={item.path} className='hover:bg-gray-100 hover:text-gray-800 px-3 py-2 rounded-lg transition-all duration-200 font-medium'>
                                        {item.name}
                                    </Link>
                                </li>
                            ))}
                        ...
                        {token ? (
                            <div className='flex items-center gap-2'>
                                <button className='p-2 hover:bg-blue-50 hover:text-blue-600 rounded-full transition-all duration-200' aria-label="Messages">
                                    <BiMessageRoundedDetail size={24} />
                                </button>
                                <button className='p-2 hover:bg-blue-50 hover:text-blue-600 rounded-full transition-all duration-200' aria-label="Notifications">
                                ...
                                <div className="relative">
                                    <button
                                        ...
                                        aria-haspopup="true"
                                    >
                                        <img src={logo} alt="Profile" className='object-cover w-full h-full' />
                                    </button>
                                    {showDropdown && (
                            ...
                            <Link to="/login" className='bg-blue-600 text-white text-sm py-2 px-5 rounded-full hover:bg-blue-700 transition-all duration-200 font-medium'>
                                Login
                            </Link>
                        )}

                        <div className='w-[1px] h-[30px] bg-gray-300' />
                        <Link to="/jobs" className='text-sm border-2 border-gray-400 py-2 px-4 rounded-full hover:bg-gray-100 hover:border-gray-500 font-semibold transition-all duration-200'>
                            + Host
                        </Link>
                    </div>
                    ...
                    <div className='lg:hidden flex items-center gap-2'>
                        <button onClick={() => setShowMobileSearch(!showMobileSearch)} className='p-2 hover:bg-gray-100 rounded-full transition-all duration-200' aria-label="Toggle search">
                        ...
                        {token && (
                            <>
                                ...
                                <div className="relative">
                                    ...
                                    {showDropdown && (
                                        <div
                                            ref={!isLargeScreen ? dropdownRef : null}
                        ...
                        <button onClick={handleMobileMenuToggle} className='mobile-menu-trigger p-2 hover:bg-gray-100 rounded-full transition-all duration-200 ml-1' aria-label="Toggle menu" aria-expanded={showMobileMenu}>
                ...
                <div className="lg:hidden fixed inset-0 backdrop-blur-xs bg-opacity-50 z-50" onClick={closeMobileMenu}>
                    ...
                    >
                        <div className="flex items-center justify-between p-4 border-b border-gray-200">
                            ...
                            <button onClick={closeMobileMenu} className='p-2 hover:bg-gray-100 rounded-full transition-all duration-200' aria-label="Close menu">
                                <MdClose size={24} />
                            </button>
                        </div>
